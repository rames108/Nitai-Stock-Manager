...
def btnAddItem():
    if request.method == "POST":
        ...
        existing_item = Item.query.filter_by(txtItemName=item_name).first()
        if existing_item:
        ...
        new_item = Item(
            ...
            txtItemPrice=price, 
            txtItemDesc=item_desc, 
            intItemStock=stock, 
            intItemMin=min_stock,
            image=image_data,
    ...
    return render_template('selling.html', user=current_user)

# Add a route to serve images
@auth.route('/image/<int:item_id>')
def serve_image(item_id):
    ...
    # Optional: return a default image if no image is found
    return '', 404


@auth.route('/remove_item/<int:item_id>', methods=['POST'])
@login_required
